...
tryCreateTables db = do
  ...
  let q =
        [ "CREATE TABLE IF NOT EXISTS users (user_id INTEGER not null primary key autoincrement, user_name TEXT not null,user_password TEXT not null, user_privilege INTEGER not null)",
          ...
          "CREATE TABLE IF NOT EXISTS restocks(restock_id INTEGER PRIMARY key autoincrement NOT NULL, restock_date DATE NOT NULL, restock_qty INTEGER NOT NULL)",
          "CREATE TABLE IF NOT EXISTS out_of_stock( oos_id INTEGER PRIMARY key autoincrement NOT NULL, oos_date DATE NOT NULL)",
          "CREATE TABLE IF NOT EXISTS restock_stock_xref( restock_id INTEGER NOT NULL, stock_id INTEGER NOT NULL, FOREIGN KEY(restock_id) REFERENCES restocks(restock_id), FOREIGN KEY(stock_id) REFERENCES stock(stock_id), CONSTRAINT rsx_id PRIMARY KEY(restock_id, stock_id))",
...
-- leastSqProd :: Connection -> Integer -> Integer -> IO Float
-- leastSqProd db pid num = do
--   conn <- clone db
--   let q1 = "SELECT restock_date FROM stock WHERE product_id = (?)"
--   let q2 = "SELECT restock_quantity FROM stock WHERE product_id = (?)"
--   let q3 = "SELECT oos_date FROM stock WHERE product_id = (?)"
--   restocks <- quickQuery' conn q1 [toSql pid]
--   oos <- quickQuery' conn q3 [toSql pid]
--   let restocks' :: [Day] = concatMap (map fromSql) restocks
--   -- let x = leastSq (fromList oosDiff) (fromList restockQty)
--   disconnect conn
--   return 1.0 -- (x $ realToFrac num)
...
makeSale db date pid quant = do
  ...
  let q5 = "INSERT INTO out_of_stock (product_id, oos_date) values (?, ?)"
  ...
  when (oos == [[toSql (0 :: Integer)]]) $ do
    _ <- quickQuery' conn q5 [toSql pid, toSql date]
    return ()
  commit conn
  disconnect conn
  return $ ok >= 1
